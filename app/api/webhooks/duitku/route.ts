...
import { type DuitkuCallbackPayload, duitkuClient } from "@/lib/duitku";
...
export async function POST(req: Request) {
  try {
    ...
    const payload = Object.fromEntries(
      formData.entries(),
    ) as unknown as DuitkuCallbackPayload;
    ...
    if (!isValidSignature) {
      console.warn(
        `[DUITKU_WEBHOOK] Invalid signature for invoice ${merchantOrderId}`,
      );
    ...
    if (resultCode === "00") {
      // Payment Successful
      console.log(
        `[DUITKU_WEBHOOK] Payment success for invoice ${merchantOrderId}`,
      );
      ...
      console.log(
        `[DUITKU_WEBHOOK] Payment failed or expired for invoice ${merchantOrderId}`,
      );
